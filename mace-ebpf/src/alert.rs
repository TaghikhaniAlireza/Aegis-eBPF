...
fn format_syscall_arguments(ev: &mace_ebpf_common::MemoryEvent) -> Vec<String> {
    match ev.event_type {
...
fn comm_string(comm: &[u8; mace_ebpf_common::TASK_COMM_LEN]) -> String {
...
mod tests {
    ...
    fn unique_yaml_path(prefix: &str) -> std::path::PathBuf {
        let unique = SystemTime::now()
            ...
            .as_nanos();
        std::env::temp_dir().join(format!("mace-alert-test-{prefix}-{unique}.yaml"))
    }
