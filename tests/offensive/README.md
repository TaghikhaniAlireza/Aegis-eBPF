...
If `mace_start_pipeline` logs **`BPF_PROG_LOAD` … `sys_enter_execve` … Permission denied**, pull the latest `main` first (recent releases fix verifier issues around `bpf_probe_read_user_str_bytes` NUL termination). If it still fails on a very strict kernel, rebuild with **`MACE_EBPF_EXECVE_ARGV0_ONLY=1`** (`Makefile` → **`rust-build-ebpf-argv0`**) so `execve` captures **`argv[0]` only**. If load **still** fails, try **`MACE_EBPF_EXECVE_NO_USER_ARGV=1`** (`rust-build-ebpf-no-user-argv` / `build-agent-ebpf-no-user-argv`): execve enter skips all user argv reads (diagnostic). Note that **`OFFENSE_GTFONODE_CHILD_PROCESS_SHELL`** matches **`argv_contains`** across multiple argv slots, so that rule typically **will not fire** on live Node execve without full argv capture; use **section A** (`mace-replay` + fixture) to validate the rule, or relax detection to patterns visible in `argv[0]` / `/proc` haystack.
