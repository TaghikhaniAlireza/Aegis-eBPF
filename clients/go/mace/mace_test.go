...
// Tests require CGO and libmace_ebpf.a (build with `cargo build -p mace-ebpf` from repo root).
...
func TestClient_defaultBufferSize(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		b, _ := json.Marshal(MaceEvent{Timestamp: uint64(i), ProcessName: "x", SyscallName: "mmap"})
		pushMaceEventJSON(string(b))
	}
	for i := 0; i < 10; i++ {
		ev := <-c.Events()
