// Command aegis-agent is the standalone Aegis eBPF security agent (systemd-friendly).
package main
...
import (
	...
	"github.com/aegis-ebpf/sdk/clients/go/internal/agentconfig"
...
func newRootCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:          "aegis-agent",
		Short:        "Aegis eBPF security agent",
		SilenceUsage: true,
...
func run(configPath string) error {
	...
	fmt.Fprintf(os.Stderr, "aegis-agent: logging security events to %s (format=%s)\n", cfg.Logging.Path, cfg.Logging.Format)

	client, err := aegis.NewClient(4096)
	if err != nil {
		return fmt.Errorf("aegis client: %w", err)
	}
	...
	if err := aegis.LoadRulesFile(cfg.Rules.Path); err != nil {
		_ = client.Close()
	...
	fmt.Fprintf(os.Stderr, "aegis-agent: engine running (rules=%q); send SIGTERM or SIGINT to stop\n", cfg.Rules.Path)
	...
	fmt.Fprintln(os.Stderr, "aegis-agent: shutdown signal received, stopping Aegis engine...")
	_ = aegis.StopPipeline()
	if err := client.Close(); err != nil {
		fmt.Fprintf(os.Stderr, "aegis-agent: client close: %v\n", err)
	}
	<-shutdown
	fmt.Fprintln(os.Stderr, "aegis-agent: stopped cleanly")
	return nil
}

func logSecurityEvent(log *logrus.Logger, ev *aegis.AegisEvent, format string) {
	if format == "json" {
