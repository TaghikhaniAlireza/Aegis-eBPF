...
use super::{
    ...
    handle::{AegisArenaHandle, AegisErrorCode},
...
/// `aegis_arena_new` pointer and is **not** consumed.
///
/// # Safety
/// `handle` must be a live arena handle. `out_stats` must be writable.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn aegis_simulate_jit_storm(
    ...
    let result = catch_unwind(AssertUnwindSafe(|| {
        // SAFETY: same opaque-pointer layout as `AegisArenaHandle::from_arc` / `as_ref` in `handle.rs`.
...
mod tests {
    ...
    use super::{super::handle::AegisArenaHandle, *};

    #[test]
    fn jit_storm_roundtrip_10k() {
        let arena = EventArena::new(4096);
        let raw = Arc::into_raw(arena) as *mut AegisArenaHandle;
        let mut stats = JitStormStats::default();
        let rc = unsafe { aegis_simulate_jit_storm(raw, 10_000, &mut stats) };
        assert_eq!(rc, AegisErrorCode::Success as i32);
