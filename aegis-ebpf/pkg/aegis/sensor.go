...
import (
	...
	"github.com/aegis-ebpf/sdk/pkg/aegis/aegisproto"
)
...
type Sensor struct {
	...
	arena        *C.AegisArenaHandle
	alertChannel *C.AegisAlertChannelHandle
...
func NewSensor(cfg Config) (*Sensor, error) {
	if cfg.ArenaCapacity <= 0 || (cfg.ArenaCapacity&(cfg.ArenaCapacity-1)) != 0 {
		return nil, fmt.Errorf("aegis: ArenaCapacity must be a positive power of two, got %d", cfg.ArenaCapacity)
	}
	if cfg.AlertChannelCapacity == 0 {
		return nil, fmt.Errorf("aegis: AlertChannelCapacity must be > 0")
	}
	if cfg.PollInterval <= 0 {
		cfg.PollInterval = DefaultConfig().PollInterval
	}

	arena := C.aegis_arena_new(C.size_t(cfg.ArenaCapacity))
	if arena == nil {
		return nil, fmt.Errorf("aegis: aegis_arena_new failed")
	}

	alertCh := C.aegis_alert_channel_new(C.size_t(cfg.AlertChannelCapacity))
	if alertCh == nil {
		C.aegis_arena_free(arena)
...
func (s *Sensor) PushEvent(e Event) error {
	...
	ce := e.toCEvent()
	code := C.aegis_arena_try_push(s.arena, &ce)
...
func (s *Sensor) Close() error {
	s.closeOnce.Do(func() {
		...
		s.mu.Lock()
		if s.arena != nil {
			C.aegis_arena_free(s.arena)
			s.arena = nil
		}
		if s.alertChannel != nil {
			C.aegis_alert_channel_free(s.alertChannel)
			s.alertChannel = nil
		}
		s.closed = true
		s.mu.Unlock()
...
func (s *Sensor) drainArenaTick() {
	for {
		...
		code := C.aegis_arena_try_pop(s.arena, &ce)
...
func (s *Sensor) recvAlertsTick(buf *[]byte) {
	for {
		...
		n, need, err := recvAlertResult(C.aegis_alert_channel_try_recv(
			s.alertChannel,
