...
3. **Execve argv limits:** eBPF captures a **bounded** argv snapshot at `sys_enter_execve` (v13 wire: header + **192**-byte NUL-separated args, max **4** args × **63** bytes of argv data per arg via a **64**-byte per-CPU read buffer). **`is_truncated`** indicates overflow; there is **no** second-chunk protocol—operators rely on haystack fallbacks (`cmdline_context`, `/proc`) only when the snapshot is incomplete. **`BPF_PROG_LOAD` EACCES** on some hosts is often **lockdown** or **capabilities**, not argv size. **Troubleshooting strict verifiers:** if load still fails after pulling latest `main`, rebuild with **`MACE_EBPF_EXECVE_ARGV0_ONLY=1`** (see `Makefile` → `rust-build-ebpf-argv0`) for a smaller `sys_enter_execve` that only reads **`argv[0]`**. If that still fails **`BPF_PROG_LOAD`**, try **`MACE_EBPF_EXECVE_NO_USER_ARGV=1`** (`rust-build-ebpf-no-user-argv`): execve enter skips all user argv reads (header-only; `is_truncated=1`). Rules that need `-e` / script argv (for example the Node GTFOBins simulation) will **not** match live until you use full multi-arg capture or **`mace-replay`** with a fixture.
