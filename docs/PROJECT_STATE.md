...
3. **Execve argv limits:** eBPF captures a **bounded** argv snapshot at `sys_enter_execve` (v13 wire: header + **192**-byte NUL-separated args, max **4** args × **64** bytes read per arg). **`is_truncated`** indicates overflow; there is **no** second-chunk protocol—operators rely on haystack fallbacks (`cmdline_context`, `/proc`) only when the snapshot is incomplete. **`BPF_PROG_LOAD` EACCES** on some hosts is often **lockdown** or **capabilities**, not argv size.
